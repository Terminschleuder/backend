...
The public read API sends CORS headers so a browser on another origin (the read-only demo
client) can call it directly:

- Methods: `GET`, `HEAD`, `OPTIONS` only (read-only).
- No credentials (`Access-Control-Allow-Credentials` is off) — these are anonymous public
  reads, so no cookies/JWT cross-origin.
- `CORS_ALLOW_ALL_ORIGINS` (default `True`, demo-friendly) echoes the request `Origin`. For a
  locked-down prod, set `CORS_ALLOW_ALL_ORIGINS=False` and list `CORS_ALLOWED_ORIGINS`. See
  `.env.example`.

### OpenAPI schema

The API is **self-describing** via an OpenAPI 3 schema generated by `drf-spectacular`:

| URL | Purpose |
| --- | --- |
| `GET /api/schema/` | raw OpenAPI 3 schema (YAML by default; `?format=json` for JSON) |
| `GET /api/schema/swagger-ui/` | interactive Swagger UI |
| `GET /api/schema/redoc/` | ReDoc docs |

External clients (the demo) generate their TypeScript types from this schema as a single
source of truth. Export a committed artifact with:
  ...
  "results": [
    {
      ...
      "ends_at": null,
      "hero_image": "http://localhost:8000/media/events/hero/berlin-python-meetup-20260903.png",
...
payload). `hero_image` is the absolute URL of an optional hero/banner image (`null` when none);
it is read-only here — uploads happen in the backoffice.
...
`source`, `promoted_from`, and `hero_image` are read-only (lifecycle/provenance are driven
by dedicated actions and the hero image is uploaded in the backoffice, not the public
payload). Returns `201` with the created event (same shape as the list item). Validation
errors → `400`.
...
| `python manage.py seed_demo` | rich, **JSON-driven** demo dataset: 20 organizations, 120 venues across 15 European cities, 8 categories, 20 sources, ingestion runs, observations, and 400 canonical events (full lifecycle + classification matrix) **with a generated hero image per event**, plus a `demo` staff user and the `ingestion` group; idempotent & non-destructive. Data lives in `events/data/seed/*.json` — regenerate it with `python3 scripts/build_demo_fixture.py`; `--no-hero` skips banner generation for a fast re-seed. |
