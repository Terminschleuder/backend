...
| --------------- | ---------------------------------------------------- | ------------------------------------------------ |
| `SECRET_KEY`    | `django-insecure-change-me`                          | **Set a strong random value in production** (`openssl rand -hex 32`). |
| `DEBUG`         | `False`                                              | Compose sets `True` for dev.                     |
| `ALLOWED_HOSTS` | `[]`                                                 | Your domain(s), comma-separated. **Empty rejects every request with a 400** — compose sets `*` for dev. |
| `DATABASE_URL`  | `postgis://terminschleuder:terminschleuder@127.0.0.1:5432/terminschleuder` | Compose overrides to the `db` host. On a container hoster, the host is the db's internal service name. |
| `CSRF_TRUSTED_ORIGINS` | `https://terminschleuder.online`, `https://www.terminschleuder.online`, `https://*.terminschleuder.online` | **Must include your domain** or every admin/POST action fails the CSRF check. Set it whenever you deploy on a different domain. |
...
| `DJANGO_JWT_SIGNING_KEY` | unset | Optional separate key for JWT signing; defaults to `SECRET_KEY`. |
...
JWT access tokens live **15 min**, refresh tokens **7 days**, signed with
`DJANGO_JWT_SIGNING_KEY` if set, otherwise `SECRET_KEY`.
...
The full production story — the four containers, their env vars, volumes, ports,
networking, and deploy/verify order — is in **[docs/deployment.md](docs/deployment.md)**.
For platform health checks, `GET /api/schema/` returning `200` is the probe.
...
The complete story — including app-platform deployments where you never type
`docker run` at all — lives in **[docs/deployment.md](docs/deployment.md)**: a topology
of who talks to whom, the full env-var list per container, volumes, health checks,
and ordering. The commands below are the plain-`docker run` version of the same
steps. **Deploy in this order: db → backend → frontend → extractor.**

```bash
# The db and backend must share a network for container-name DNS to work.
docker network create ts-net

# 1. Database (pin by release version — `latest` floats with a Postgres major bump).
#    Remember these three values — the backend's DATABASE_URL must use the SAME
...
# Verify: container stays up (docker logs ts-db shows "PostgreSQL init process complete").
...
#    ALLOWED_HOSTS must be your domain (empty = every request 400s); set
#    CSRF_TRUSTED_ORIGINS for any domain other than terminschleuder.online.
...
# Verify: GET /api/schema/ → 200 · GET /api/cities/ → 2131 cities · /admin/ login works.
# First boot is slower — watch for "[entrypoint] Bootstrap complete" in docker logs.

# 3. Frontend: takes NO env vars (the browser calls the backend, not this container).
#    Publish 8080; point the API URL at the backend in the app's Settings page.
docker run -d --name ts-app --restart unless-stopped -p 8080:8080 \
  ghcr.io/terminschleuder/frontend:<release-version>
# Verify: the page loads; set the API base URL in Settings and the event list fills in.

# 4. Extractor onboarding (one-time; secrets are shown once by design, so bootstrap
#    deliberately does NOT create it from env). The backoffice path — log into
#    /admin/ as the bootstrapped superuser, create the "extractor" service account
#    in the ingestion group, mint its API key — works on ANY hoster, no exec needed.
#    On a host where you can run one-off containers, the command equivalent is:
docker run --rm --network ts-net \
  -e SECRET_KEY=… -e DEBUG=False -e ALLOWED_HOSTS=… -e DATABASE_URL=postgis://terminschleuder:<db-password>@ts-db:5432/terminschleuder \
  ghcr.io/terminschleuder/backend:<release-version> \
  sh -c "ENTRYPOINT_SKIP_TASKS=1 python manage.py create_service_account extractor --group ingestion"
# Then mint the key via /admin/ as above — it becomes the extractor's EXTRACTOR_API_KEY.
# Run the extractor itself with its env (EXTRACTOR_API_KEY, EXTRACTOR_API_BASE_URL,
# EXTRACTOR_LLM_BASE_URL/_MODEL/_API_KEY, EXTRACTOR_STATE_FILE + a /app/state volume):
docker run -d --name ts-extractor --restart unless-stopped \
  ...
  -e EXTRACTOR_LLM_BASE_URL=<llm-endpoint>/v1 -e EXTRACTOR_LLM_MODEL=<model> -e EXTRACTOR_LLM_API_KEY=<llm-key> \
  -e EXTRACTOR_STATE_FILE=/app/state/crawl_state.json \
  ghcr.io/terminschleuder/extractor:<release-version>
```

`ts-db` / `ts-web` / `ts-app` / `ts-extractor` are just names for the shared
network's DNS — on an app-platform UI the platform's internal service names take
their place inside `DATABASE_URL` (see [docs/deployment.md](docs/deployment.md)).
